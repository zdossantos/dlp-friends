...
type TranslationKey = 'locale.label' | 'locale.fr' | 'locale.en' | 'navigation.settings' | 'navigation.profile' | 'navigation.discovery';

const translationFor = (messages: TranslationMessages, key: TranslationKey): string => {
...
export function useTranslations(): {
    ...
    const t = (key: TranslationKey, replacements: Record<string, string | number> = {}): string =>
        Object.entries(replacements).reduce(
            (message, [replacement, value]) => message.replaceAll(`:${replacement}`, String(value)),
    ...
    return {
        locale,
        t,
        formatDate: (value, options) => new Intl.DateTimeFormat(locale.value, options).format(new Date(value)),
